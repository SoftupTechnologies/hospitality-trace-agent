...
#### Import the Rulebook to Google Sheets

1. Open your Google Drive account
2. Navigate to **Google Sheets** and create a new spreadsheet named **"Rulebook"**
3. Download and open the `Rulebook.xlsx` file from this repository
...
1. In the **Get System Rule Book** node:
   ...
   - In the **Document** field, select your **Rulebook** spreadsheet
...
**Quick Setup:**
1. In your Supabase project, go to **Connect > API Keys** and copy the **Project URL** (this is your **Host**)
2. Go to **Settings > API Keys** and reveal and copy the **Secret** key (this is your **Service Role Secret**)
3. In n8n, create a new Supabase credential with these values
...
3. Import the workflow JSON files in this order (sub-workflows first):
   - `lock_workflow/Manage Lock Create and Wait.json` - Lock management sub-workflow
   - `lock_workflow/Release Lock n8n.json` - Lock release sub-workflow
   - `ai_agent_workflow/Sub Trace Ai Agent.json` - AI processing sub-workflow
   - `booking_workflow/Booking Traces.json` - Main booking webhook flow
   - `reservation_workflow/Reservation Traces.json` - Main reservation webhook flow
   - `reporting_workflow/Daily Traces Email.json` - Daily email report flow
...
|------------|---------------|---------------|---------------------|
| `PROPERTY_ID` | booking_workflow/Booking Traces.json | If is the valid property node | Your Apaleo property ID to filter incoming webhooks |
| `PROPERTY_ID` | reservation_workflow/Reservation Traces.json | If is the valid property node | Your Apaleo property ID to filter incoming webhooks |
...
| Placeholder | Workflow File | Node/Location | What to Replace With |
|------------|---------------|---------------|---------------------|
| `REDACTED:REDACTED` | ai_agent_workflow/Sub Trace Ai Agent.json | Sweeply Basic Auth Token node | Your Sweeply credentials in format `username:password` (will be base64 encoded automatically) |
| Google account | ai_agent_workflow/Sub Trace Ai Agent.json | Get System Rule Book & Get Property Rule Book nodes | Your Google account credential (sign in via Google from n8n) |

#### Daily Traces Email Workflow

| Placeholder | Workflow File | Node/Location | What to Replace With |
|------------|---------------|---------------|---------------------|
| `ACCOUNT_ID` | reporting_workflow/Daily Traces Email.json | Account details node (JavaScript) | Your Apaleo account ID |
| `PROPERTY_ID` | reporting_workflow/Daily Traces Email.json | Account details node (JavaScript) | Your Apaleo property ID |
| `EMAIL_ADDRESS` | reporting_workflow/Daily Traces Email.json | Account details node (JavaScript) | Recipient email address(es) for daily reports |
| `Bearer xxxxxxxx` | reporting_workflow/Daily Traces Email.json | Send email node → Headers | Your Resend API key (format: `Bearer re_xxxxx`) |
| `Your Company Name <no-reply@yourdomain.com>` | reporting_workflow/Daily Traces Email.json | Send email node → Body Parameters | Your verified sender email address |
